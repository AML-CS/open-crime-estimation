...
PATH = 'app/routes/open_crime_estimation/'

def prepareData(dataPath,departamento,ciudad,sexo,edad,mes,dia,hora):
    weights = pd.read_csv(PATH + 'Data_Parms/'+dataPath+'.csv_Params.csv',sep=',')
    map_cat = pd.read_csv(PATH + 'Data_encoder/'+dataPath+'.csv_encoder_info.csv',sep=',')
    geo_loc = pd.read_csv(PATH + 'Data_por_barrio_V2/'+departamento+"-"+ciudad+' (CT)'+'.csv',sep=',')
...
def plotDataHTML(df_plot,departamento,ciudad):
    ...
    geomap.save(PATH + 'Mapa'+departamento+'-'+ciudad+'.html')
    return(PATH + 'Mapa'+departamento+'-'+ciudad+'.html')
